...
hittable_list random_scene() {
  ...
  */
  ...
  world.add(make_shared<moving_sphere>(point3(0, 1, 0), point3(0, 1, 0), 0, 1, 1,
                                       material1));

  auto material2 = make_shared<lambertian>(color(0.4, 0.2, 0.1));
  world.add(make_shared<moving_sphere>(point3(-4, 1, -2), point3(-4, 1, 2), 0,
                                       1, 1, material2));
...
#pragma omp parallel for
  ...
  for (int j = image_height - 1; j >= 0; j--) {
    for (int i = 0; i < image_width; i++) {
      write_color(std::cout, pixel_matrix[j][i], samples_per_pixel);
    }
  }
  std::cerr << "\nDone.\n";
  return 0;
